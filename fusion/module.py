...
class Embedding(torch.nn.Module):
    ...
    def init_embedding(self):
        rel_file = "data/embeddings/rel_embed.pkl"
        embed_file = "/hits/basement/nlp/liuwi/resources/embeddings/glove.840B.300d.txt"
        rel_vectors = build_embedding_of_corpus(
            embed_file=embed_file,
            vocab=self.vocab,
            embed_dim=300,
            saved_file=rel_file
        )
        rel_vectors = torch.tensor(rel_vectors)
        assert rel_vectors.size(0) == len(self.vocab), (rel_vectors.size(0), len(self.vocab))
        assert rel_vectors.size(1) == self.embed_dim, (rel_vectors.size(1), self.embed_dim)

        self.embedding.weight.data.copy_(rel_vectors)
...
class FusionTransformerLayer(nn.Module):
    def __init__(self, hidden_size, num_heads, dropout=0.1, scaled=True):
        super(FusionTransformerLayer, self).__init__()
...
class FusionTransformer(nn.Module):
    def __init__(self, params):
        ...
        self.dropout = 0.1
        self.scaled = params["scaled"]
        self.layers = nn.ModuleList(FusionTransformerLayer(
            self.hidden_size, self.num_heads, self.dropout, self.scaled
        ) for _ in range(self.num_layers))
